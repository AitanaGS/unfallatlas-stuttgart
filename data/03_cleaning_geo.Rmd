...
arten <- c("Zusammenstoß mit anfahrendem / anhaltendem / ruhendem Fahrzeug",
         "Zusammenstoß mit vorausfahrendem / wartendem Fahrzeug",
         "Zusammenstoß mit seitlich in gleicher Richtung fahrendem Fahrzeug",
         "Zusammenstoß mit entgegenkommendem Fahrzeug",
         "Zusammenstoß mit einbiegendem / kreuzendem Fahrzeug",
...
address_to_remove <-", (Stuttgart, )?Baden-Württemberg, \\d{5}, Deutschland"
...
accidents_final <- accidents_geo_clean_select %>%
  ...
  mutate(monatn = fct_relevel(monatn, monate),
         ...
         licht2 = case_when(
           licht == "Dämmerung" | licht == "Dunkelheit" ~ "Dämmerung/Dunkelheit",
           licht == "Tageslicht" ~ "Tageslicht",
           is.na(licht) ~ NA_character_
         ),
         licht2 = fct_relevel(licht2, c("Dämmerung/Dunkelheit", "Tageslicht")),
         istsonst2 = case_when(
           istgkfz == "Unfall mit Güterkraftfahrzeugbeteiligung" |
             istsonst == "Unfall mit Beteiligung eines oben nicht genannten Verkehrsmittels" 
           ~ "Unfall mit Beteiligung eines anderen Verkehrsmittels",
           ((istgkfz == "Unfall ohne Güterkraftfahrzeugbeteiligung" |
             is.na(istgkfz)) & 
              (istsonst == "Unfall ohne Beteiligung eines oben nicht genannten Verkehrsmittels" |
                 is.na(istsonst))) 
           ~ "Unfall ohne Beteiligung eines anderen Verkehrsmittels"
         ),
         strzust = fct_relevel(strzust, strzustand),
         strzust2 = case_when(
           strzust == "nass/feucht/schlüpfrig" | strzust == "winterglatt" ~ "nass/glatt",
           strzust == "trocken" ~ "trocken",
           is.na(strzust) ~ NA_character_
         ),
         strzust2 = fct_relevel(strzust2, c("nass/glatt", "trocken")),
         datum = format(make_date(jahr, monat), "%Y-%m-%d"),
         istradb = if_else(istrad == 'Unfall mit Fahrradbeteiligung', TRUE, FALSE),
         istpkwb = if_else(istpkw == 'Unfall mit PKW-Beteiligung', TRUE, FALSE),
         istfussb = if_else(istfuss == 'Unfall mit Fußgängerbeteiligung', TRUE, FALSE),
         istkradb = if_else(istkrad == 'Unfall mit Kraftradbeteiligung', TRUE, FALSE),
         istgkfzb = if_else(istgkfz == 'Unfall mit Güterkraftfahrzeugbeteiligung', TRUE, FALSE),
         istsonstb = if_else(istsonst == 'Unfall mit Beteiligung eines oben nicht genannten Verkehrsmittels', TRUE, FALSE),
         istsonst2b = if_else(istsonst2 == 'Unfall mit Beteiligung eines anderen Verkehrsmittels', TRUE, FALSE),
  ...
  mutate(zeit = fct_relevel(zeit, zeiten),
         art = if_else(art == "Zusammenstoß mit anfahrendem/ anhaltendem/ruhendem Fahrzeug",
                       "Zusammenstoß mit anfahrendem / anhaltendem / ruhendem Fahrzeug", art),
         art = fct_relevel(art, arten)) %>%
  mutate(address2 = str_replace_all(as.character(address), address_to_remove, "")) %>%
  relocate(zeit, .after = stunde) %>%
  relocate(kateg2, .after = kateg) %>%
  relocate(licht2, .after = licht) %>%
  relocate(istsonst2, .after = istsonst) %>%
  relocate(strzust2, .after = strzust) %>%
  relocate(datum, .after = monatn) %>%
  relocate(istradb, .after = istrad) %>%
  relocate(istpkwb, .after = istpkw) %>%
  relocate(istfussb, .after = istfuss) %>%
  relocate(istkradb, .after = istkrad) %>%
  relocate(istgkfzb, .after = istgkfz) %>%
  relocate(istsonstb, .after = istsonst) %>%
  relocate(istsonst2b, .after = istsonst2) %>%
  relocate(address2, .after = address)
